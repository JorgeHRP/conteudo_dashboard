...
import auth
import config
from data.loader import start_loader, aplicar_filtros, get_cursos, get_ies
...
def create_app():
    ...
    def api_filtros():
        ...
        return jsonify({
            "regioes":       uniq(cursos, "NO_REGIAO"),
            "ufs":           uniq(cursos, "SG_UF"),
            "areas":         uniq(cursos, "NO_CINE_AREA_GERAL"),
            "redes":         uniq(cursos, "TP_REDE"),
            "graus":         uniq(cursos, "TP_GRAU_ACADEMICO"),
            "org_academica": uniq(ies,    "TP_ORGANIZACAO_ACADEMICA"),
            "microrregioes": uniq(cursos, "NO_MICRORREGIAO"),
            "municipios":    uniq(cursos, "NO_MUNICIPIO"),
        })
    ...
    def api_ies_top():
        df  = aplicar_filtros(get_cursos(), get_filters())
        ies = get_ies()[["CO_IES","NO_IES","SG_IES","SG_UF_IES","TP_ORGANIZACAO_ACADEMICA"]].copy()
        r   = df.groupby("CO_IES")[["QT_MAT","QT_ING","QT_CONC"]].sum().reset_index()
        r   = r.merge(ies, on="CO_IES", how="left").sort_values("QT_MAT", ascending=False).head(15)
        total = r["QT_MAT"].sum() or 1
        r["market_share"] = (r["QT_MAT"] / total * 100).round(2)
        return jsonify(r.rename(columns={
